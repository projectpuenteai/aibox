...
Companion files:
  - app_payloads.py — pydantic request models and input normalizers
  - app_routes.py — register_routes(app, rt) for every /v1/app/* endpoint
"""
import base64
import csv
import hashlib
...
from app_payloads import _coerce_bool, normalize_language_preference
...
class StorageRuntime:
    ...
    def __init__(self, llama_base_url: str):
        ...
        # Citation links resolve through Kiwix's search endpoint rather than a
        # direct article URL, because the retrieved chunk title is often a
        # section heading or near-match that doesn't exist as its own page.
        # The search page always renders something useful for the student.
        ...
        self.embed_model = os.getenv("EMBED_MODEL", "/models/embed-m3").strip() or "/models/embed-m3"
    ...
    def _language_directive(self, response_language: Optional[str] = None) -> str:
        ...
        Chinese mid-stream. Both branches explicitly forbid Chinese/CJK output
        because Qwen 2.5 is heavily Chinese-trained and will otherwise drift,
        especially on short, ambiguous, or proper-noun-heavy prompts.
        """
        lang = (response_language or "").strip().lower()
        if lang == "es":
            return (
                "IMPORTANTE: Debes responder SIEMPRE en español. El idioma del estudiante es español. "
                "No uses inglés, chino, ni ningún otro idioma en tu respuesta bajo ninguna circunstancia. "
                "Nunca incluyas caracteres chinos (汉字), japoneses ni coreanos en tu respuesta."
            )
        # Default to English when language is unset, unknown, or "en". Qwen 2.5
        # without an explicit anchor will sometimes emit Chinese, so we always
        # return a directive (never an empty string).
        return (
            "IMPORTANT: You MUST respond in English only. Do not use Chinese, Spanish, or any other language. "
            "Never include Chinese characters (汉字), Japanese, or Korean characters in your response under any circumstances."
        )
    ...
    def _build_retrieval_system_message(self, context: str, response_language: Optional[str] = None, rag_index_language: Optional[str] = None) -> str:
        ...
        if is_es:
            ...
            parts.append("Recordatorio final: tu respuesta debe estar 100% en español. No uses chino ni ningún otro idioma.")
        else:
            # English path needs the same anchor — Qwen 2.5 will otherwise emit
            # Chinese characters mid-response, especially on long contexts or
            # proper-noun-heavy queries.
            parts.append("Final reminder: your response must be 100% in English. Do not use Chinese, Japanese, Korean, or any other non-English script.")
        return "\n\n".join(parts).strip()
    ...
    def _build_wiki_citation(self, title: Any, base_url: Optional[str], wiki_language: Optional[str] = None) -> Optional[Dict[str, str]]:
        ...
        book = self.kiwix_book_es if language == "es" else self.kiwix_book_en
        encoded_title = quote(page_title, safe="")
        encoded_book = quote(book, safe="")
        query = f"books.name={encoded_book}&pattern={encoded_title}" if book else f"pattern={encoded_title}"
        wiki_url = f"{root}/wiki/{language}/search?{query}" if root else f"/wiki/{language}/search?{query}"
        return {
    ...
    def inject_wiki_context(self, base_messages: List[Dict[str, str]], context: str, missing_reason: Optional[str] = None, response_language: Optional[str] = None, rag_index_language: Optional[str] = None) -> List[Dict[str, str]]:
        if context.strip():
            ...
            lang_directive = self._language_directive(response_language)
            # Always inject the language directive even when no BASE_SYSTEM_PROMPT
            # is configured — otherwise Qwen 2.5 has no anchor and can drift to
            # Chinese, which is the whole reason _language_directive exists.
            if base and lang_directive:
                content = f"{base}\n\n{lang_directive}".strip()
            elif base:
                content = base
            else:
                content = lang_directive
            if not content:
...
def mount_app_storage(app, llama_base_url: str):
    ...
    # Surface missing RAG model paths AT MOUNT TIME so operators see them in
    # the container's first log lines, not only after the background startup
    # thread tries to load them and /health silently stays at 503.
    embed_model_path = Path(rt.embed_model)
    if not embed_model_path.exists():
        logger.error(
            "Embedding model not found at %s. RAG will fail to come up and "
            "/health will stay at 503. Check EMBED_MODEL env var and /models mount.",
            embed_model_path,
        )
    rerank_model_path = Path(rt.rerank_model)
    if not rerank_model_path.exists():
        logger.error(
            "Reranker model not found at %s. RAG will fail to come up and "
            "/health will stay at 503. Check RERANK_MODEL env var and /models mount.",
            rerank_model_path,
        )
    chroma_dir = Path(rt.chroma_persist_dir)
    if not chroma_dir.exists():
        logger.error(
            "Chroma index directory not found at %s. RAG cannot serve queries. "
            "Check CHROMA_PERSIST_DIR env var and /chroma_db mount.",
            chroma_dir,
        )
    if getattr(rt, "load_es_index_at_startup", False):
        chroma_dir_es = Path(rt.chroma_persist_dir_es)
        if not chroma_dir_es.exists():
            logger.error(
                "Spanish Chroma index directory not found at %s. Spanish RAG "
                "will fail to come up. Check CHROMA_PERSIST_DIR_ES and the "
                "chroma_db_es_native named volume.",
                chroma_dir_es,
            )
    rt.init_db()
    rt.seed_admin()
